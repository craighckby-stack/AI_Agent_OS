...
Before the kernel dispatches a request to this module, it must pass the following pre-flight checks defined in `diagnostic_hook.py`:
1. **Provider Availability**: Validates that the configured LLM endpoint key is present in the environment.
2. **Context Persistence**: Verifies that the memory layer for fallback history is accessible or writable.
...
[INTEGRATION_PATH: ./diagnostic_hook.py]